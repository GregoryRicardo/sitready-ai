...
def prepare_firestore_test_data():
    ...
    if not emulator_host:
        raise RuntimeError(
            ...
            "Start the local Firestore emulator before running tests."
        )
    ...
    if environment != "local":
        raise RuntimeError(
            f"SITEREADY_ENV must be 'local' for tests, got '{environment}'."
        )
    ...
    if not c002.exists or not c003.exists:
        raise RuntimeError(
            "Firestore test setup completed, but C002/C003 "
            "could not be read back from the emulator."
        )
...
def _clear_taskmaster_state(db, contractor_ids: set[str]) -> None:
    """
    Remove generated Taskmaster actions and approvals for the
