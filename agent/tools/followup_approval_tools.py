...
def _build_proposed_action(issue: dict) -> dict:
    """
    Convert a readiness issue into a deterministic proposed
    follow-up action.
...
def _normalize_proposed_actions(
    proposed_actions: list[dict],
) -> list[dict]:
    """
    Normalize proposed actions into a stable representation so
    identical proposals produce identical fingerprints.
    """
...
def _build_approval_fingerprint(
    contractor_id: str,
    proposed_actions: list[dict],
) -> str:
    """
    Create a deterministic SHA-256 fingerprint for a contractor's
    proposed action set.
    """

    payload = {
        "contractor_id": contractor_id,
        "proposed_actions": _normalize_proposed_actions(
            proposed_actions
        ),
    }
...
def _find_existing_pending_approval(
    ...
    """
    Find an existing pending approval for the same contractor
    and identical proposed action set.
    """
    ...
    for document in query:
        ...
        if (
            ...
            and existing.get("approval_fingerprint")
...
def propose_followup_actions(
    contractor_id: str,
) -> dict:
    """
    Assess contractor readiness and prepare follow-up actions
    for human approval.

    IMPORTANT:
    This function does NOT create follow-up actions.

    If an identical pending approval already exists, the existing
    approval is returned instead of creating a duplicate approval.
    """
    ...
    proposed_actions = [
        _build_proposed_action(issue)
        for issue in assessment["issues"]
    ]

    normalized_actions = _normalize_proposed_actions(
        proposed_actions
    )

    approval_fingerprint = _build_approval_fingerprint(
        contractor_id=contractor_id,
        proposed_actions=normalized_actions,
    )

    existing_pending = _find_existing_pending_approval(
        contractor_id=contractor_id,
    ...
    if existing_pending:
        return {
            ...
            "proposed_actions": existing_pending[
                "proposed_actions"
            ],
    ...
    approval_id = (
        f"APR-{now.strftime('%Y%m%d%H%M%S%f')}"
    )
...
def approve_followup_actions(
    approval_id: str,
    approved_by: str = "human_reviewer",
) -> dict:
    """
    Approve a pending follow-up request and create the proposed
    actions in Firestore.

    This is the only function in this workflow that performs
    the actual follow-up action creation.
    """
    ...
    for proposed_action in approval.get(
        "proposed_actions",
        [],
    ):
